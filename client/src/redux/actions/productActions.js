...
export const listProductsFiltered = (seachValue = '') => async (dispatch) => {
    try {
        dispatch({ type: PRODUCT_LIST_REQUEST });

        const products = await ProductService.getAll();
        const filteredProducts = products.filter((product) => product.name.toLowerCase().includes(seachValue.toLowerCase()));
